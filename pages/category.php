...
<body class="d-flex flex-column">
    ...
    <main class="col-12 bg-light">
        ...
        if($type == "parent"){
            $sql = "Select p.id, p.bezeichnung, p.beschreibung, p.preis, p.bild From Produkt p, Kategorie k WHERE k.parentid = ? and p.kategorieid = k.id";
            $statement = $mysqli->prepare($sql);
            $statement->bind_param("i", $category);
        }else if($type == "child"){
            $sql = "Select p.id, p.bezeichnung, p.beschreibung, p.preis, p.bild FROM Produkt p, Kategorie k WHERE p.kategorieid = k.id AND k.id = ?";
            $statement = $mysqli->prepare($sql);
            $statement->bind_param("i", $category);
        }else{
