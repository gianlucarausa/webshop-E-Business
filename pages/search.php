...
<head>
    <title>Suche | Tritschler Webservices</title>
    <meta charset="UTF-8">
...
<?php
    ...
    if(!isset($_GET["search"])) {
        ...
        $sql = "SELECT * FROM Produkt WHERE bezeichnung LIKE ? OR beschreibung LIKE ?";
        $statement = $mysqli->prepare($sql);
        $wildcard = "%" . $searchParam . "%";
        $statement->bind_param("ss", $wildcard, $wildcard);
        $statement->execute();
